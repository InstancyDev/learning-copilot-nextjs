...
function loadDocument(callback) {
	...
	$.ajax({
		type: 'POST',
		url: getApplicationPath('documentInformation'),
		data: JSON.stringify(data),
		global: false,
		contentType: "application/json",
		headers: {
...
function printPdf() {
	if (documentGuid != "" && typeof documentGuid != "undefined") {
		...
		request.open('POST', getApplicationPath('print'), true);
...
var fnscroll= function () {
	...
	console.log('scroll initialize called');
	$('#gd-pages').scroll(function (e) {	
		console.log(e);
